...
DECLARE
	...
	derived_work_type TEXT;
BEGIN
	IF TG_TABLE_NAME = 'base_task' THEN
		NEW.work_type := 'TASK';
	ELSE
		derived_work_type := UPPER(REPLACE(REGEXP_REPLACE(TG_TABLE_NAME, '^[^_]+_', ''), '_', ' '));
		NEW.work_type := COALESCE(NULLIF(BTRIM(COALESCE(NEW.work_type, '')), ''), NULLIF(BTRIM(derived_work_type), ''), 'TASK');
	END IF;
