ALTER TABLE base_task
	DROP CONSTRAINT IF EXISTS chk_base_task_type;

DROP TRIGGER IF EXISTS trg_base_task_before_write ON base_task;

UPDATE base_task
SET work_type = 'TASK'
WHERE COALESCE(NULLIF(BTRIM(work_type), ''), '') <> 'TASK';

ALTER TABLE base_task
	ALTER COLUMN work_type SET DEFAULT 'TASK';

ALTER TABLE base_task
	ADD CONSTRAINT chk_base_task_type CHECK (BTRIM(work_type) = 'TASK');
