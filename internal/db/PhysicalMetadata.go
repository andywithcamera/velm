	...
	"_ai_connection": {
		{
			...
			LABEL: "API Key",
			IS_HIDDEN: false,
			// Virtual columns (e.g. write-only secrets) are consumed by applyBuiltinRecordTransforms
		// before the post-transform ValidateFormWrite pass, so they must
		// never be form-required — IS_NULLABLE=false would fail that pass
		// after the value has been translated into api_key_enc.
			IS_NULLABLE: true,
		},
	},
}
